...
int main(void)
{
  ...
  openmv_spi_init();
  move_servo(0, 1500, 300);
  move_servo(1, 1500, 300);
  HAL_Delay(400);
  move_servo(0, 1100, 300);
  move_servo(1, 1900, 300);
  HAL_Delay(400);
  move_servo(0, 1900, 300);
  move_servo(1, 1100, 300);
  HAL_Delay(400);
  ...
  /* add threads, ... */
  osThreadDef(fputTask, StartFputTask, osPriorityLow, 0, 1024);
  ...
  osThreadDef(servoTrackTask, StartServoTestTask, osPriorityAboveNormal, 0, 256);
  servoTrackTaskHandle = osThreadCreate(osThread(servoTrackTask), NULL);
  if (servoTrackTaskHandle == NULL)
  {
    printf("SERVO_TASK_CREATE_FAILED\r\n");
  }
  else
  {
    printf("SERVO_TASK_CREATE_OK\r\n");
  }
...
void StartServoTestTask(void const * argument)
{
    printf("\r\n=== SERVO TEST TASK START ===\r\n");
    for(;;)
    {
        printf("SERVO_STEP:CENTER\r\n");
        move_servo(0, 1500, 200);
        move_servo(1, 1500, 200);
        osDelay(600);

        printf("SERVO_STEP:LEFT\r\n");
        move_servo(0, 1100, 300);
        move_servo(1, 1100, 300);
        osDelay(700);

        printf("SERVO_STEP:RIGHT\r\n");
        move_servo(0, 1900, 300);
        move_servo(1, 1900, 300);
        osDelay(700);
    }
